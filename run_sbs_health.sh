## This is the script to start health script and it will also send email notifications
platform="STAGING"
function notify_all_thru_email()
{
        cat $2 |mail -s "[$platform]$1" -a "From: jcs.sbsnotifications@zmail.ril.com" shishir.gowda@ril.com chirag.aggarwal@ril.com rahul4.jain@ril.com ravikanth.maddikonda@ril.com vivek.kayarohanam@ril.com sandeep41.kumar@ril.com souvik.ray@ril.com
}
...
function notify_thru_email()
{
	...
	if [[ "$time" -ge 1215 ]] && [[ "$time" -le 1218 ]];then
	        ...
	        cat $2 |mail -s "[$platform]$1" -a "From: jcs.sbsnotifications@zmail.ril.com" sandeep41.kumar@ril.com chirag.aggarwal@ril.com
	fi
}
script_interval=60
...
do
	...
	if [[ "$time" -le 0100 ]] ;then
	    ...
	    then
	      ...
	      notify_all_thru_email "Instance API daily summary" $file_summary
	      to_send=0
	      passCount=0
	      failCount=0
	      rm -rf $file_summary
	...
	then
	    ...
	    time=`date +"%H-%M-%S"`
	    file_to_clean="/tmp/$day/$time"
	    grep "Attached\|Detached\|Created\|Deleted" $file >> $file_to_clean
	    echo -e "Clean up needed for file $file_to_clean" >>$file
            notify_all_thru_email "ALERT: Instance API failed" $file
	    failures=$(($failures+1))
	else
	    ...
	    echo -e "end time `date`" >> $file
            if (($failures!=0))
	    then
                notify_all_thru_email "Instance API success" $file
	    else		
